...
12. Calculating and plotting **home range** area metrics using
    ...
    c)  Brownian bridges
13.  **ESRI Shapefile** export and import 
...
``` {r, include=TRUE, eval=FALSE}
install.packages(c("sp","rgdal","rgeos","adehabitatLT","adehabitatHR","leaflet","tidyverse","devtools"), repos='http://cran.us.r-project.org')
```
...
b1 <- blacktip[blacktip$Transmitter.Name%in%levels(blacktip$Transmitter.Name)[1],]
b2 <- blacktip[blacktip$Transmitter.Name%in%levels(blacktip$Transmitter.Name)[2],]
b3 <- blacktip[blacktip$Transmitter.Name%in%levels(blacktip$Transmitter.Name)[3],]
...
The %.N% call is a special variable that holds the number of rows in that current group. Grouping by `Transmitter` obtains the number of rows (i.e. detections) for each tranmitter. We can include multiple variables in the `by` parameter to subset our datset further:
...
blacktip %>% 
  subset(`Transmitter Name` == "Colin") %>% # subset dataset to include only detections by 'Colin'
  with(table(`Receiver`)) %>% # Create a table with the number of rows (i.e. detections) per receiver
...
* This function recognises multiple date time formats and parses them to a standardised 'POSIX' format that R uses (`ymd` for dates; `ymd_hms` for date and time parsing)
* essential when working with spatio-temporal datasets like telemetry data

Currently in our `blacktip` dataset the "Date and Time (UTC)" column are recognised as "characters" and recorded in the Universal Coordinated Time Zone (UTC). We want to convert this to local time (UTC + 10 hours). Lets use `lubridate` to convert this column into the 'POSIX' format and into local date time.
...
blacktip %>%
  ...
  summarise(DailyDetections= n()) %>% ## use pipes to calculate numbers of detections per day per animal
...
blacktip %>%
  ...
  xlab("Date") + ylab("Receiver") +
  geom_point() +
  facet_wrap(~`Transmitter Name`, ncol=1) ## This time plot seperate boxplots for each shark
```
...
![Close up of one of our spatial obects](Images/banner1.jpg)
...
We can also now add the locations of our tagged sharks to show their position within the array. (Note I'm concerned here that by using the raw detection data we may break Leaflet if too many detections are loaded)
...
```{r, eval = FALSE}
saveWidget(myleafletplot, file="mymap.html") # uses the htmldidgets package
```

#Session 3: Accessing the temporal component of the tracking dataset
...
# We can have a look at the unique transmitter codes using VTrack's ExtractUniqueValues() function
...
The file will be written in your Project folder and can be opened by double clicking the file. Users can adjust the time slider to visualise individual time periods for display or can click the spanner icon to slow down the speed of the animation.


There is also a second handy function for visualising the movements of multiple animals at a time.
...
```{r}
VR2ArrayDM <- GenerateDirectDistance(VR2Array) 
VR2ArrayDM
```
...
We would then use this within the RunResidenceExtraction() function in VTrack to extract the movements between hydrophonme stations and link this to our measure of distance travelled
...
Generating these distance matrices can also be a useful way of extracting the distance of each receiver stations to a specific location (i.e. receiver positioned at the mouth of an estuary, instream structure or other point of interest). These distances can then be merged with the original detection dataset and plotted through time to look at associations and commonality in the timing of animal movements.

```{r}
DistDownstream <- CircuitousDM[,1]
names(DistDownstream) <- c("STATIONNAME","Distance.DS")

Vcrocsm <- merge(Vcrocs,DistDownstream,by="STATIONNAME")
...
Vcrocsm %>%
ggplot(daes(x=DATETIME, y=DistDownstream,group=TRANSMITTERID)) +
  geom_line(alpha=0.5) +
  scale_y_continuous(name="Distance from Downstream (km)")+
  xlab("Date") +
  theme_bw()+
...
barra <- subset(barra[,1]<as.POSIXct("2016-01-31 14:00:00"))
...
GenerateRasterDistance(sPointsFile = POINTS3,
                       sTransition = tr)
