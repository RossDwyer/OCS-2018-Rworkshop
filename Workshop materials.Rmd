...
jacko$dt <- lubridate::ymd_hms(jacko$Date.Time)
jacko <- jacko[order(jacko$dt),]
...
jacko_move<- move(x = jacko$LONGITUDE, y = jacko$LATITUDE, time = jacko$dt,
                  proj = CRS("+proj=longlat +datum=WGS84"), data = jacko)
```
...
animate_move(jacko_move, out_dir, conv_dir = conv_dir,
             paths_mode = "true_data", frames_nmax = 50, layer="basemap",
             img_title = img_title, img_sub = img_sub, out_format = "mp4")

# Note: for this demonstration we have limited the animation to 100 frames
...
In this last session, we will be working with passive telemetry data from three Tiger sharks (*Galeocerdo cuvier*) that were monitored around Heron and One Tree Islands, and was kindly contributed for this workshop by [Dr. Michelle Heupel](http://data.aims.gov.au/staffcv/jsf/external/view.xhtml?partyId=900003933). Sharks were monitored over three years as part of a larger study examining the ecology and movement patterns of reef predators.

We will use this dataset to  



```{r, echo=FALSE, message=FALSE, include=TRUE, results=FALSE}
require(leaflet)
require(sp) 
require(raster)
require(knitr)
