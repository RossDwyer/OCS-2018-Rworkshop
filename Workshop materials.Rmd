...
```{r, out.width = 1000, fig.retina = NULL, echo=FALSE}
knitr::include_graphics("Images/tiger_banner.png")
```
...
source("R Code/COA.R")
coa.ll<- COA(tiger[tiger$Transmitter.Name%in%"Tony",], "Transmitter.Name", 60)
...
leaflet() %>%
  ...
  addPolygons(data=kud, color="white", weight=2, group="Fixed KUD", fillOpacity = 0) %>%
  
  #addRasterImage(kbb.ras, colors = p2, group="Brownian Bridge KUD", opacity = 0.4) %>%
  addPolygons(data=kbb, color="white", weight=2, group="Brownian Bridge KUD", fillOpacity = 0) %>%
