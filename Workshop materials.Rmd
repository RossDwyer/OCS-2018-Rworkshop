...
9.  Calculating and plotting **home range** area metrics
a)  MCPs (array/matrix)
...
3. R can easily handle large datasets
4. that in R, it is relativelty easy to produce amazing images and to share these with collaborators
...
ggplot2 is a plotting system for R, that takes care of many of the fiddly details that commonly make plotting graphs in R a hassle. It also provides a powerful model of graphics that makes it easy to produce complex multi-layered graphics.
...
To do this, we use the lubridate package, and state what datetime format our data is in using the `format = "%"` statement. Note that in this case we have the R default format of dates plus times: "Y-m-d H:M:S". 

In the following example, we subset our data for only those locations that occurred before a particular date. To do this we will use the `XXXXXXXX` function in the lubridate package to confirm the string type, and the `mutate` funtion to add this column to our existing data frame
