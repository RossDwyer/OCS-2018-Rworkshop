...
# Session 1: Importing and handling telemetry data in **R**
...
We can also now add the locations of our tagged sharks to show their position within the array. 
...
## Lets plot detection data from "Ana"
ana <-
  blacktip %>%
  subset(`Transmitter.Name` == "Ana") %>%
  remove.duplicates() ## remove duplicated points to reduce the number of points to plot on our leaflet map


## Now lets add Ana's detections to our leaflet map
...
## You can try adding detection data from "Colin" and "Bruce" to the map
```
...
![Wenlock array](Images/receivers and raster.jpeg)
...
# Session 4: Quantifying home ranges using passive telemetry
